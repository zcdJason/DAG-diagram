<template>
  <div class="page-content" @mousedown="startNodesBus($event)" @mousemove="moveNodesBus($event)"
    ...
    <div class="page-left">
      ...
      <el-tree :data="leads" :props="defaultProps" @node-click="handleNodeClick"/>
    </div>
    <div class="headbar">
    ...
    <DAGBoard :inputFlowStatus="inputFlowStatus" :DataAll="DataAll" @updateDAG="updateDAG"
              @editNodeDetails="editNodeDetails" @doSthPersonal="doSthPersonal"/>
    <node-bus v-if="dragBus" :value="busValue.value" :pos_x="busValue.pos_x" :pos_y="busValue.pos_y"/>
    <editor ref='myEditor' class="json-editor" v-model="jsonEditor" :options="options" @init="editorInit" lang="json"
            theme="chrome" width="400" height="100%"/>
  </div>
</template>
    ...
    methods: {
      startTimer() {
        this.logTimer = window.setInterval(() => {
          console.log("flow-status 状态变化, current flow status:", this.inputFlowStatus);
          this.inputFlowStatus = !this.inputFlowStatus;
        }, 10000)
      },
      ...
      updateDAG(data, actionName) {
        ...
        switch (actionName) {
          case "startRunning":
            this.inputFlowStatus = true;
            break;
          case "pauseRunning":
            this.inputFlowStatus = false;
            break;
          default:
          // console.log('actionName not found->', actionName)
        }
      },
      saveChange() {
